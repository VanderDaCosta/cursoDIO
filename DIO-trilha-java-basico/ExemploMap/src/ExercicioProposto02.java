...
public class ExercicioProposto02 {
    public static void main(String[] args) {
        ...
        Random geradorLancamentos = new Random();
        for (int i = 0; i < quantLancamentos; i++){
            int number = geradorLancamentos.nextInt(6) + 1;
            valores.add(number);
        }
        ...
        //conteúdo do for + try/cath - totalmente opcional - porém não sei fazer de outro jeito kk
        for(int i = 0; i < 3; i++){
            try{
                Thread.sleep(3000);
                System.out.println(".");
